...
<<<<<<< Updated upstream
> Generated from `CLAUDE.md` by `install.sh --agent=codex`.
> Codex CLI reads this file at session start. Skills are installed separately under `~/.codex/skills/<prefix>-<name>` and mirrored in the target vault under `.codex/skills/`.
=======
> Generated from `CLAUDE.md` by agent adapters.
> Agents that read `AGENTS.md` should treat this as the neutral bridge to the
> operational vault. Agent-specific integrations, such as Codex skills, are
> installed separately in their own runtimes.
