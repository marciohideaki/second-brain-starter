...
for old in "$GEMINI_DIR"/*-GEMINI.md; do
  [ -f "$old" ] || continue
  if grep -qF '<!-- Second Brain managed bridge -->' "$old" \
    && grep -qF "<!-- Vault: $TARGET_VAULT -->" "$old"; then
    rm -f "$old"
  fi
done

sed "s|{VAULT}|$TARGET_VAULT|g" "$ADAPTER/templates/gemini.md" > "$DEST"
printf '\n<!-- Second Brain managed bridge -->\n' >> "$DEST"
printf '\n\n<!-- Vault: %s -->\n\n' "$TARGET_VAULT" >> "$DEST"
...
if [ ! -f "$GEMINI_DIR/GEMINI.md" ] || grep -qF '# Gemini CLI managed index' "$GEMINI_DIR/GEMINI.md"; then
...
EOF
  ...
  echo "  - $GEMINI_DIR/GEMINI.md already exists (kept)"
fi
